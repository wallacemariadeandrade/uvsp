...
B="";
M = string(zeros(1,7)); 
labelConc = []; // Labels das Concentrações
labelConc2 = []; // Auxiliar das concentrações
...
// leitura do arquivos com os dados
...
labelConc(N-p+1) = "Concentração da diluição " + string(p) + " (mol/dm^3)"; // Criando os labels referente a cada solução
labelConc2(p) = ""; // Para preenchimento dos campos das concentrações
conc(p, 1) = string(p); // Prenchendo os valores de x do vetor concentração
tamanho = size(a);
tamanho = tamanho(1); 
...
// Ajustando o ultimo valor de comprimento de onda
Base(amp, 1) = 1000;

// Entrada das Concentrações

Conc(:,2) = x_mdialog("Digite as concentrações", labelConc, labelConc2);
...
title("Reta de regressão - " + "UV-Vis " + nomeAmostra, "fontsize", 3); // Adiciona um título ao gráfico
...
//     Edição do gráfico
    ...
    title("UV-Vis " + nomeAmostra,"fontsize", 3);
// ------------------------------------------
